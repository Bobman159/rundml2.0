...
/**
 * Serialize SQLType Java objects into SQL text clauses.
 *
 */
public class SQLTypeSerializer {
	 ...
	 */
	public final String serializePredicatesList(List<IPredicate> predList) {
		
		StringBuilder sql = new StringBuilder();
		for (IPredicate pred : predList) {
	 ...
	 */
	public final String serializeSQLTypeList(List<ISQLType> exprList) {
	
		StringBuilder csvString = new StringBuilder();
		for (ISQLType exprBase : exprList) {
			csvString.append(serialize(exprBase));
			if (exprList.lastIndexOf(exprBase) + 1 < exprList.size()) {
				csvString.append(",");
			}
		}
	...
	public final String serialize(ICaseClause caseStmt) {
		...
		sql.append(serializeCaseWhenThenList(caseStmt.getWhenThenConditions())).append(" ");
		if (caseStmt.hasElse()) {
	 ...
	 */
	public final String serializeCaseWhenThenList(List<ICaseWhenThen> streamWhenThen) {
		StringBuilder sql = new StringBuilder();
